...
public class DNSResolver implements Runnable {
	...
	public boolean resolveLocal(String client, DatagramPacket request, DatagramPacket response) throws IOException {
		...
		if(type != 1 && type != 28)
			return handle_NonTyp1_Type28(client, dnsQuery, response);
	...
	private boolean handle_NonTyp1_Type28(String client, SimpleDNSMessage dnsQuery, DatagramPacket response) {
		...
		DNSResponsePatcher.logNstats(true, host);
		int length = dnsQuery.get_NonTyp1_Type28_FilterResponse(response.getData(), response.getOffset());
		response.setLength(length);
		return true;
	}
